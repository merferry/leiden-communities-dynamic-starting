...
\caption{Our Dynamic-supporting Parallel Leiden \cite{sahu2024dflouvain}.}
\label{alg:leiden}
\begin{algorithmic}[1]
...
\Function{leiden}{$G^t, C^{t-1}, K^t, \Sigma^t, F$} \label{alg:leiden--begin}
  ...
  \ForAll{$l_p \in [0 .. \text{\small{MAX\_PASSES}})$} \label{alg:leiden--passes-begin}
    ...
    \State $G' \gets$ Aggregate communities in $G'$ using $C'$ \Comment{Alg. \ref{alg:leidenag}} \label{alg:leiden--aggregate}
